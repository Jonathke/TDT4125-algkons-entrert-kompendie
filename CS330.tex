...
Vi får et resultat som sier at $\sum\nolimits_{t=1}^{T}\sum\nolimits_{i=1}^{N}v_t(i)p_t(i) \geq (1-\epsilon)\sum\nolimits_{t=1}^{T}v_t(j)-\frac{1}{\epsilon}ln(N)$ for $\epsilon \leq \frac{1}{2}$. Dette finner vi ved å finne øvre å nedre grenser for det første uttrykket, og så triske litt med ligningene. Ikke spesielt vanskelig å forstå, men ganske mye å skrive, så se evt. boka/forelesning. Det sentrale å merke seg er at denne grensen bryr seg ikke om fordelinger eller den slags. Dermed holder denne grense selv om vi velger verdier "adversarily".
